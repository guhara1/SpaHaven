...
의존성 없이 Node.js만으로 `docs/` 아래에 전체 사이트가 생성됩니다.
GitHub Pages에서 **Settings → Pages → Source: `docs/` 폴더**로 바로 배포할 수 있습니다.
...
| 항목 | 현재 값 | 설명 |
|---|---|---|
| `SITE_URL` | `https://spahaven.example.com` | 실제 도메인으로 교체 (canonical·og·sitemap·schema에 사용) |
...
`docs/assets/img/hero.jpg` 경로에 원하는 이미지를 넣으면 메인 히어로와 og:image에 자동으로 노출됩니다.
